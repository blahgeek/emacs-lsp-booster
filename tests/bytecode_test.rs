...
fn run_one_test(json_str: &str, object_type: bytecode::ObjectType) -> Result<()> {
    ...
    let bytecode = bytecode::generate_bytecode_repl(
        &json_value,
    ...
    eprintln!(
        "Json: {} bytes, Bytecode: {} bytes, ratio={}",
        json_str_nowhitespaces.len(),
        bytecode.len(),
    ...
    let elisp_code = format!(
        include_str!("./benchmark_and_compare.template.el"),
        json_file.display(),
...
fn test_bytecode() {
...

    {
        ...
        let arr = (0..100000)
            .map(|x| json::Value::from(&format!("{}", x)))
            .collect::<Vec<_>>();
        let value = json::Value::from(json::Array::from(arr));
...

    {
        ...
        let map = (0..100000)
            .map(|x| (format!("x{}", x), json::Value::from(x)))
            .collect::<HashMap<_, _>>();
        let value = json::Value::from(json::Object::from_iter(map.iter()));
        run_one_test(&value.to_string(), bytecode::ObjectType::Plist).unwrap();
    }
}
