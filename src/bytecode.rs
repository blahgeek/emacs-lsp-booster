...
impl BytecodeCompiler {
    ...
    fn compile_value_map_plist_or_alist(&mut self, map: &json::Object, alist: bool) {
        ...
        for (key, value) in map {
            if alist {
                self.compile_constant_op(LispObject::Symbol(key.to_string()));
                self.compile_value(value);
                self.ops.push(Op::Cons);
            } else {
                self.compile_constant_op(LispObject::Keyword(key.to_string()));
                self.compile_value(value);
            }
        }
    ...
    fn compile_value_map_hashtable(&mut self, map: &json::Object) {
        ...
        for (key, value) in map {
            ...
            self.compile_constant_op(LispObject::Str(key.to_string()));
            self.compile_value(value);
    ...
    fn compile_value(&mut self, value: &json::Value) {
        match value.get_type() {
            ...
            json::JsonType::Number => {
                if value.is_f64() {
                    self.compile_constant_op(LispObject::Float(value.to_string()));
                } else {
                    self.compile_constant_op(LispObject::Int(value.as_i64().unwrap()));
                }
            }
            json::JsonType::String => {
                self.compile_constant_op(LispObject::Str(value.as_str().unwrap().to_string()));
            }
            json::JsonType::Array => {
                self.compile_value_array(value.as_array().unwrap().as_slice());
            }
    ...
    fn into_bytecode(self) -> Result<(Vec<u8>, Vec<LispObject>, i32)> {
        ...
        let index_remap = constants_array
            .iter()
            .enumerate()
            .map(|(new_idx, (_, idx_and_count))| (idx_and_count.0, new_idx as u32))
            .collect::<BTreeMap<_, _>>();

        let mut constants_array = constants_array
            .into_iter()
            .map(|(obj, _)| obj)
            .collect::<Vec<_>>();
        // rearrange constants
        ...
        while constants_array.len() > CV_NORMAL_SLOT_COUNT as usize {
            let len = {
                let remaining = (constants_array.len() - CV_NORMAL_SLOT_COUNT as usize)
            ...
            let v = constants_array
                .splice(
                    (constants_array.len() - len)..constants_array.len(),
                    Vec::new(),
                )
                .collect();
